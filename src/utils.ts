...
export function validateApplicationIdAndSecret(
  ...
  try {
    ...
    if (wallet.address !== applicationId) {
      throw new InvalidParamError(
        `Invalid applicationId and applicationSecret passed to the constructor.`
      );
    }
  } catch (error) {
    throw new InvalidParamError(
...
export function validateURL(url: string, functionName: string): void {
  ...
  try {
    ...
    throw new InvalidParamError(
      `Invalid URL format passed to ${functionName}.`
    );
  }
}
...
export async function fetchAppById(appId: string): Promise<string> {
  ...
  try {
    ...
    const response = await fetch(
      `${APP_BACKEND_URL}/api/applications/sdk/get-zk-enabled-app/${appId}`
    );
